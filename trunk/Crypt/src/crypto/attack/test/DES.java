...
public class DES implements crypto.util.FunctionType {
	...
	public int calc(int input) {
		...
		desFunc(workingKey, t1, 0, t2, 0);
		return getIthBit(t2[s], b);
	}
	...
	public DES(int _s, int _b) {
		s = _s;
		b = _b;
		workingKey = generateWorkingKey(true, Hex.decode("0123456789abcdef"));
		t1 = new byte[16];
		t2 = new byte[16];
		bb = new int[32];
		for (int i = 0; i < 32; ++i)
