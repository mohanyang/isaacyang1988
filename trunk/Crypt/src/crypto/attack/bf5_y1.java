 ...
 * 
 */
public class bf5_y1 implements FunctionType {
	...
	public bf5_y1() {
		...
		for (int i = 0; i < (1 << n); ++i) {
			 ...
			 * x4 + x5 + x6 + x7:
			 */
			arrayTable[i] = (tmp[5] * tmp[3] * tmp[2] * tmp[0])
					^ (tmp[5] * tmp[4] * tmp[2] * tmp[0])
					...
					^ (tmp[4] * tmp[3] * tmp[2]) ^ (tmp[5] * tmp[2] * tmp[0])
					^ (tmp[5] * tmp[2] * tmp[1]) ^ (tmp[5] * tmp[4] * tmp[1])
					^ (tmp[5] * tmp[4] * tmp[2]) ^ (tmp[5] * tmp[4] * tmp[3])
					^ (tmp[6] * tmp[2] * tmp[0]) ^ (tmp[6] * tmp[2] * tmp[1])
					^ (tmp[6] * tmp[3] * tmp[0]) ^ (tmp[6] * tmp[4] * tmp[0])
					^ (tmp[6] * tmp[4] * tmp[1]) ^ (tmp[6] * tmp[4] * tmp[3])
					^ (tmp[6] * tmp[5] * tmp[1]) ^ (tmp[6] * tmp[5] * tmp[3])
					^ (tmp[6] * tmp[5] * tmp[4]) ^ (tmp[7] * tmp[2] * tmp[1])
					^ (tmp[7] * tmp[3] * tmp[2]) ^ (tmp[7] * tmp[5] * tmp[1])
					^ (tmp[7] * tmp[5] * tmp[4]) ^ (tmp[7] * tmp[6] * tmp[0])
					^ (tmp[7] * tmp[6] * tmp[3]) ^ (tmp[7] * tmp[6] * tmp[5])
					^ (tmp[1] * tmp[0]) ^ (tmp[2] * tmp[1]) ^ (tmp[3] * tmp[1])
					^ (tmp[4] * tmp[1]) ^ (tmp[4] * tmp[3]) ^ (tmp[5] * tmp[1])
					^ (tmp[5] * tmp[2]) ^ (tmp[5] * tmp[3]) ^ (tmp[6] * tmp[1])
					^ (tmp[7] * tmp[0]) ^ (tmp[7] * tmp[5]) ^ (tmp[7] * tmp[6])
					^ (tmp[1]) ^ (tmp[3]) ^ (tmp[4]) ^ (tmp[5]) ^ (tmp[6])
					^ (tmp[7]);
		}
	}
