...
public class bfIDEA implements FunctionType {
	...
	private void show(byte[] t) {
		for (int i = 0; i < 8; ++i)
			System.out.print(to(t[i]));
		System.out.println();
	}

	private String to(byte x) {
		String ret = Integer.toHexString(x & 0xff);
		if (ret.length() == 1)
			ret = "0" + ret;
		return ret;
	}
