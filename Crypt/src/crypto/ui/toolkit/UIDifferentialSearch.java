...
public class UIDifferentialSearch extends BasicToolkit {
	static public final long serialVersionUID = 183;

	@Override
	public Component subConstruct() {
		JLabel orderLabel, inLabel;
		final JTextField orderTextField, inTextField;
		JButton inButton;

		orderLabel = new JLabel("Differential Order");
		...
		orderTextField.getDocument().addDocumentListener(
				new DocumentListener() {
					@Override
					public void changedUpdate(DocumentEvent arg0) {
						ArgManager.put("order", orderTextField.getText());
					}
		...
		if (ArgManager.get("order") != null) {
			orderTextField.setText(ArgManager.get("order"));
		}
		if (ArgManager.get("in") != null) {
			inTextField.setText(ArgManager.get("in"));
		}
		orderLabel.setLabelFor(orderTextField);
		...
		mainPanel
				.setToolTipText("This test will search for all the possible bases which lead to constant differential");
		...
		gridbag.setConstraints(orderTextField, c);
