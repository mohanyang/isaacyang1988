...
openOutputStreams(int option)
{
	...
	sprintf(freqfn, "experiments/%s/freq", generatorDir[option]);
	if ( (freqfp = fopen(freqfn, "w")) == NULL ) {
		printf("\t\tMAIN:  Could not open freq file: experiments/%s/freq", generatorDir[option]);
		exit(-1);
	}
	if ( (summary = fopen("finalAnalysisReport", "w")) == NULL ) {
		printf("\t\tMAIN:  Could not open stats file: experiments/%s/finalAnalysisReport", generatorDir[option]);
		exit(-1);
	}
